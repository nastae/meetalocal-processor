...
public class LiveEmailSenderTest {
    ...
    public void send_VolunteerRequest_ShouldSend() {
        final EmailSenderConfig config = configFactory.getVolunteerRequestConfig(sampleLiveMeet(), "TOKEN", WEBSITE_URL);
    ...
    public void send_TouristCancellation_ShouldSend() {
        final EmailSenderConfig config = configFactory.getLocalCancellationConfig(sampleLiveMeet(), WEBSITE_URL);
        emailSender.send(new Mail(RECEIVER), config);
    }

    @Test
    public void send_TouristCancellationNotSelected_ShouldSend() {
        final EmailSenderConfig config = configFactory.getLocalCancellationNotSelectedConfig(sampleLiveMeet(), WEBSITE_URL);
        emailSender.send(new Mail(RECEIVER), config);
    }
    ...
    public void send_VolunteerCancellation_ShouldSend() {
        final EmailSenderConfig config = configFactory.getVolunteerCancellationConfig(sampleLiveMeet(), WEBSITE_URL);
        emailSender.send(new Mail(RECEIVER), config);
    }
    ...
    public static LiveMeet sampleLiveMeet() {
        return new LiveMeet(
    ...
    public static OnlineMeet sampleOnlineMeet() {
        return new OnlineMeet(
                ...
                "name", "surname", "meetalocaltest@gmail.com", Purpose.RELOCATION,
                "Lithuania" , new Date(2025, 11, 11),
                new Time(20, 10, 10), "19-20", "123", "asd",
                new HashSet<>(),
                ImmutableSet.<MeetLanguage>builder()
                        .add(new MeetLanguage(Language.ENGLISH, null))
                        .build(),
                new HashSet<>(), new HashSet<>(), sampleVolunteer(), Status.NEW, false, "1");
    }
